// The luau-lsp server's own settings, sent via workspace/configuration and
// didChangeConfiguration. Ported as data only (key/default/type) — the React
// app's generic Setting/field UI framework these also feed isn't ported, so
// there's no settings screen for these; the server just gets sane defaults.
...
export const LUAU_SETTINGS: LuauSetting[] = [
    ...
    {
        // Off to match Studio, where the new solver is still an opt-in beta.
        // Defaulting it on made the editor disagree with the engine: measured
        // on a 120-file project, V2 reported 2941 type errors against the old
        // solver's 1486 — 1455 squiggles for code Studio accepts. Turn this on
        // only alongside Studio's "New Luau Type Solver" beta.
